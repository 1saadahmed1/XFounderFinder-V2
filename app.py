...
async def main():
    ...
    if not RAPIDAPI_KEY or not GEMINI_API_KEY:
        st.error("API keys not found. Please set `RAPIDAPI_KEY` and `GEMINI_API_KEY` in `.streamlit/secrets.toml`.")
        return
    ...
    if fetch_button and input_username:
        ...
        with st.spinner("Collecting network data... This may take several minutes."):
            try:
                ...
                # Correctly call the data processor, which returns a single NetworkData object
                network_data_object = await data_processor.collect_network_data(
    ...
    if st.session_state.first_degree_profiles or st.session_state.second_degree_profiles:
        ...
        if find_candidates_button and user_prompt:
            ...
            with st.spinner("Searching for candidates with AI..."):
                try:
                    # Pass only the second-degree profiles to the AI for analysis
                    ai_results = await ai_client.search_for_candidates(
